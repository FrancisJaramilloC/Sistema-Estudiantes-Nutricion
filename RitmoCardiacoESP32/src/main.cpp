...
void saveMqttCredentials(const String& broker, int port, const String& user, const String& pass, const String& devId);
...
void connectWiFiOnly();

void mqttCallback(char* topic, byte* payload, unsigned int length) {
  String topicStr = String(topic);
  String cmdTopicSuffix = "/comandos";
  if (!topicStr.endsWith(cmdTopicSuffix)) return;
  ...
  if (strcmp(comando, "reset") == 0) {
    Serial.println("[MQTT] Comando 'reset' recibido. Borrando configuración...");
    wm.resetSettings();
    preferences.begin("sensor", false);
    preferences.clear();
    preferences.end();
    Serial.println("[MQTT] NVS limpiado. Reiniciando en modo AP...");
    delay(1000);
    ESP.restart();
  }
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("==========================================");
  Serial.println("  Monitor de Ritmo Cardíaco v3.1");
  Serial.println("  WiFiManager + MQTT + Auto-AP");
  Serial.println("==========================================");
  ...
  if (registered && mqttBroker.length() > 0) {
    Serial.println("[BOOT] Modo: REGISTRADO → conectando WiFi + MQTT");
    connectWiFiOnly();
  } else {
    Serial.println("[BOOT] Modo: NO REGISTRADO → forzando portal WiFiManager");
    setupWiFiManager();
  }
...
void connectWiFiOnly() {
  Serial.println("[WIFI] Conectando a WiFi con credenciales guardadas...");
  WiFi.mode(WIFI_STA);
  WiFi.begin();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("[WIFI] Forzando portal WiFiManager...");
    registered = false;
    preferences.begin("sensor", false);
...
void setupWiFiManager() {
  Serial.println("[WIFI] Iniciando WiFiManager en modo portal...");
  ...
  if (pairingCode.length() == 0) {
    ...
    Serial.println("[WARN] El dispositivo se reiniciará en modo AP para que ingreses el código.");
    delay(2000);
    ESP.restart();
  }
}
  ...
  if (httpCode > 0) {
    ...
    if (httpCode == 200 || httpCode == 201) {
      ...
      if (!error) {
        ...
        const char* devId = respDoc["device_id"] | "";
        ...
        if (strlen(broker) > 0 && strlen(mqttUser) > 0 && strlen(mqttPass) > 0) {
          ...
          saveMqttCredentials(mqttBroker, mqttBrokerPort, mqttUsername, mqttPassword, deviceId);
        }
      }
...
bool connectMqtt() {
  ...
  while (!mqttClient.connected() && attempts < 3) {
    if (mqttClient.connect(clientId.c_str(), mqttUsername.c_str(), mqttPassword.c_str())) {
      ...
      String cmdTopic = String(MQTT_TOPIC_PREFIX) + "/" + mqttUsername + "/comandos";
      mqttClient.subscribe(cmdTopic.c_str());
      Serial.print("[MQTT] Suscrito a comandos: ");
      ...
      if (state == 4 || state == 5) {
        Serial.println("[MQTT] Credenciales rechazadas. Borrando y reiniciando en modo AP...");
        preferences.begin("sensor", false);
        preferences.clear();
        preferences.end();
        delay(1000);
...
void publishReading(int bpm, const String& timestamp) {
  ...
  String topic = String(MQTT_TOPIC_PREFIX) + "/" + mqttUsername + "/lecturas";
  topic.toLowerCase();
